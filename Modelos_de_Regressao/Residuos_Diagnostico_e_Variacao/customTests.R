...
# Retorna TRUE se o usuário calculou um valor igual ao calculado pela expressão dada.
calculates_same_value <- function(expr){
  e <- get("e", parent.frame())
  # Calcula o que o usuário deveria ter feito.
  eSnap <- cleanEnv(e$snapshot)
  val <- eval(parse(text=expr), eSnap)
  isTRUE(all.equal(val, e$val))
}

# Retorna TRUE se o usuário calculou um valor igual a algum dos calculados pelas
# expressões dadas.
